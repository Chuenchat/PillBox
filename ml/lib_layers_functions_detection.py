...
from lib_fhun_utils_box_utils import decode, nms
    ...
    def forward(self, predictions):
        ...
        for i in range(num):
            ...
            for cl in range(1, self.num_classes):
                ...
                # just very fast debug QuQ
                thr = self.conf_thresh
                while True:
                    c_mask = conf_scores[cl].gt(thr).nonzero().view(-1)   
                    if len(c_mask) != 0:
                        break                  
                    thr /= 10
