...
from lib_layers_functions_prior_box import *
from lib_layers_modules_focal_loss import *
from lib_layers_modules_l2norm import *
from lib_layers_modules_multibox_loss import *

from lib_utils_timer import Timer
from lib_utils_data_augment import preproc
from lib_modeling_model_builder import create_model
from lib_dataset_dataset_factory import load_data
from lib_utils_config_parse import cfg
from lib_utils_eval_utils import *
from lib_utils_visualize_utils import *

from lib_fhun_layers_modules_multibox_loss import *
from lib_fhun_utils_log import save_loss, save_mAP, save_multiple_loss
...
class Solver(object):
    ...
    def __init__(self):
        ...
        print('Is on CUDA: ', next(self.model.parameters()).is_cuda)
    ...
    def train_model(self):
        ...
        for epoch in iter(range(start_epoch+1, self.max_epochs+1)):
            ...
            print('\rEpoch {epoch:d}/{max_epochs:d}:\n'.format(epoch=epoch, max_epochs=self.max_epochs))
            if epoch > warm_up:
            ...
            if 'eval' in cfg.PHASE and epoch % 5 == 0:
    ...
    def train_epoch(self, model, data_loader, optimizer, criterion, writer, epoch, use_gpu):
        ...
        for iteration in iter(range((epoch_size))):
            ...
            # backprop
            optimizer.zero_grad()
            loss_l, loss_c, loss_o = criterion(out, targets)
            ...
            loss = loss_l + loss_c
            loss += loss_o if epoch > 1000 else 0
            loss.backward()
            optimizer.step()
            ...
            occ_loss += loss_o.item()

            # log per iter
            log = '\r==>Train: || {iters:d}/{epoch_size:d} in {time:.3f}s [{prograss}] || loc_loss: {loc_loss:.4f} cls_loss: {cls_loss:.4f} occ_loss: {occ_loss:.4f}\r'.format(
                    prograss='#'*int(round(10*iteration/epoch_size)) + '-'*int(round(10*(1-iteration/epoch_size))), iters=iteration, epoch_size=epoch_size,
                    time=time, loc_loss=loss_l.item(), cls_loss=loss_c.item(), occ_loss=loss_o.item())
        ...
        log = '\r==>Train: || Total_time: {time:.3f}s || loc_loss: {loc_loss:.4f} conf_loss: {conf_loss:.4f} occ_loss: {occ_loss:.4f} || lr: {lr:.6f}\n'.format(lr=lr,
                time=_t.total_time, 
                loc_loss=loc_loss/epoch_size, conf_loss=conf_loss/epoch_size, 
                occ_loss=occ_loss/epoch_size)
        # sys.stdout.write(log)
        print(log)
        sys.stdout.flush()
        ...
        # add write training loss
        save_multiple_loss(path = os.path.join(cfg['LOG_DIR'], "training_loss.csv"),
                  epoch= epoch, 
    ...
    def eval_epoch(self, model, data_loader, detector, criterion, writer, epoch, use_gpu):
        ...
        # for iteration in iter(range((10))):
            ...
            # loc, conf, occ
            out = (out[0], 
                   model.softmax(out[1].view(-1, model.num_classes)),
            	   out[2])
        ...
        # remove the untrained classes excluded "__background__"
        _label, _score, _npos = [], [], []
        for l, s, n in zip(label[1:], score[1:], npos[1:]):
            if len(l) == 0:
                continue
            _label.append(l)
            _score.append(s)
        ...
        # calc mAP
        prec, rec, ap = cal_pr(_label, _score, _npos)
        ...
        # add write training loss
        save_multiple_loss(path = os.path.join(cfg['LOG_DIR'], "testing_loss.csv"),
                  epoch= epoch, 
                  loc_loss= loc_loss/epoch_size, 
                  conf_loss= conf_loss/epoch_size,
                  occ_loss= occ_loss/epoch_size)

        save_mAP(path = os.path.join(cfg['LOG_DIR'], "testing_mAP.csv"),
                  epoch= epoch, 
                  mAP= ap)
