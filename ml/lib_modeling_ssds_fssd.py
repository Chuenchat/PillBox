...
class FSSD(nn.Module):
    ...
    def __init__(self, base, extras, head, features, feature_layer, num_classes):
        ...
        self.occluded = nn.ModuleList(head[2])
    ...
    def forward(self, x, phase='eval'):
        ...
        for (x, l, c, o) in zip(pyramids, self.loc, self.conf, self.occluded):
            loc.append(l(x).permute(0, 2, 3, 1).contiguous())
            conf.append(c(x).permute(0, 2, 3, 1).contiguous())
            occluded.append(o(x).permute(0, 2, 3, 1).contiguous())
        loc = torch.cat([o.view(o.size(0), -1) for o in loc], 1)
        conf = torch.cat([o.view(o.size(0), -1) for o in conf], 1)
        occluded = torch.cat([o.view(o.size(0), -1) for o in occluded], 1)

        if phase == 'eval':
            output = (
                loc.view(loc.size(0), -1, 4),                   # loc preds
                self.softmax(conf.view(-1, self.num_classes)),  # conf preds
                occluded.view(occluded.size(0), -1, 1),
            )
        else:
            output = (
...
def add_extras(base, feature_layer, mbox, num_classes, version):
    ...
    for layer, depth, box in zip(feature_layer[1][0], feature_layer[1][1], mbox):
        ...
        occluded_layers += [nn.Conv2d(in_channels, box, kernel_size=3, padding=1)]
    return base, extra_layers, (feature_transform_layers, pyramid_feature_layers), (loc_layers, conf_layers, occluded_layers)
