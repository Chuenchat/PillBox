# #@title lib_utils_data_augment.py
...
import random
import math
from lib_utils_box_utils import matrix_iou
...
def _crop_square(image, boxes, labels, ious):
    height, width, _ = image.shape
    
    if len(boxes)== 0:
        return image, boxes, labels

    box_l = int(random.uniform(0.5,.998) * min(height, width))
 
    x1, y1, x2, y2 = random.choice(boxes)        
    box_x = sorted([0, int(random.uniform(x2-box_l, x1)), width])[1]
    box_y = sorted([0, int(random.uniform(y2-box_l, y1)), height])[1]    
    
    roi = np.array((box_x, box_y, box_x + box_l, box_y + box_l))
    ...
    return image_t, boxes_t, labels_t, ious_t
...
def _distort(image):
    ...
    if random.randrange(2):
        _convert(image, beta=random.uniform(-80, 40))

    if random.randrange(2):
        _convert(image, alpha=random.uniform(0.4, 1.6))

    image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)

    # hue shift
    if random.randrange(2):
        tmp = image[:, :, 0].astype(int) + random.randint(-30, 30)
        tmp %= 180
        image[:, :, 0] = tmp

    # satuation amphifies
    if random.randrange(2):
        _convert(image[:, :, 1], alpha=random.uniform(0.4, 1.6))
...
def _color_transform(image):

    # shuffle rgb channels
    color_channels = cv2.split(image)
    random.shuffle(color_channels) 
    image = cv2.merge(color_channels)
...
def _expand(image, boxes, fill, p):
    ...
    for _ in range(50):
        ...
        min_ratio = max(1/1.3, 1./scale/scale)
        max_ratio = min(1.3, scale*scale)
        ratio = math.sqrt(random.uniform(min_ratio, max_ratio))
        ws = scale*ratio
        hs = scale/ratio
        if ws < 1 or hs < 1:
...
def _vertical_flip(image, boxes):
...
class preproc(object):
    ...
    def __call__(self, image, targets=None, real_data= True):
        # some bugs 
        ...
        if self.p == -1: # eval
            ...
            targets = np.hstack((boxes_t, labels_t, ious_t))
        ...
        ious_o = targets_o[:,5:6]
        boxes_o[:, 0::2] /= width_o
        boxes_o[:, 1::2] /= height_o
        # labels_o = np.expand_dims(labels_o,1)
        targets_o = np.hstack((boxes_o,labels_o,ious_o))

        image_t = image.copy()
        # image_t, boxes, labels = _crop(image, boxes, labels)
        image_t, boxes, labels, ious = _crop_square(image_t, boxes, labels, ious)
        image_t = _distort(image_t)
        # image_t = _color_transform(image_t)
        # image_t = _elastic(image_t, self.p)
        # image_t, boxes = _expand(image_t, boxes, self.means, self.p)
        image_t, boxes = _mirror(image_t, boxes)
        image_t, boxes = _vertical_flip(image_t, boxes)
        ...
        targets_t = np.hstack((boxes_t, labels_t, ious_t))
